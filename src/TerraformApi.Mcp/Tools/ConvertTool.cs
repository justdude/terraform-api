...
public static class ConvertTool
{
    ...
    public static async Task<string> Convert(
        ...
        [Description("Optional target environment name (e.g. 'dev'). Defaults to the {environment} placeholder tag.")] string? environment = null,
        [Description("Optional Terraform variable group name. Defaults to the {api-group} placeholder tag.")] string? apiGroupName = null,
        [Description("Optional APIM resource group name. Defaults to the {stage-group-name} placeholder tag.")] string? stageGroupName = null,
        [Description("Optional APIM instance name. Defaults to the {apim-name} placeholder tag.")] string? apimName = null,
        [Description("Optional API path prefix. Defaults to the {api-path-prefix} placeholder tag.")] string? apiPathPrefix = null,
        [Description("Optional API path suffix. Defaults to the {api-path-suffix} placeholder tag.")] string? apiPathSuffix = null,
        [Description("Optional API gateway hostname. Defaults to the {api-gateway-host} placeholder tag.")] string? apiGatewayHost = null,
        [Description("Optional backend service path segment. Defaults to the {backend-service-path} placeholder tag.")] string? backendServicePath = null,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(templateProfileName))
            {
                ...
                var (normalizedSettings, defaultedTags) = ApimPlaceholders.Normalize(settings);
                var configuration = openApiParser.Parse(resolvedJson, normalizedSettings);
                ...
                return ApimPlaceholders.BuildHeaderComment(defaultedTags) + apimWriter.Write(document);
            }
